...
test("Mission & Vision president photos use administrator-only private storage and render only after the CMS publishes their internal reference", async () => {
  ...
  assert.match(adminPage, /Best fit:/);
  assert.match(adminPage, /Vertical framing/);
  assert.match(adminPage, /Image size/);
  assert.match(adminPage, /presidentPhotoFocusY/);
  assert.match(adminPage, /presidentPhotoZoom/);
  ...
  assert.match(adminLibrary, /safeMissionPhotoFrameValue/);
  assert.match(missionPage, /missionVision\.presidentPhotoUrl/);
  assert.match(missionPage, /objectPosition/);
  assert.match(missionPage, /presidentPhotoZoom/);

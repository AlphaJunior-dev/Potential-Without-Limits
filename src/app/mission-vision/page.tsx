...
export default function MissionVisionPage() {
  ...
  return <div className="bg-[#FCFCFA]">
    ...
    <section className="border-y border-[#0B2E6B]/8 bg-white px-5 py-20 sm:px-8 sm:py-28 lg:px-10"><div className="mx-auto max-w-7xl"><EditorialSplit number="01" eyebrow="A founding perspective" title={missionVision.foundersTitle || "A foundation built around possibility."} aside={missionVision.presidentPhotoUrl ? <div className="relative z-10 h-full min-h-[320px] overflow-hidden"><Image src={missionVision.presidentPhotoUrl} alt={`Portrait of ${missionVision.foundersTitle || "the PWLIF president"}`} fill sizes="(min-width: 1024px) 48vw, 100vw" unoptimized className="object-cover object-center" /><div className="absolute inset-0 bg-gradient-to-t from-[#061D45]/45 via-transparent to-transparent" /></div> : <div className="relative z-10 flex h-full min-h-[320px] items-end p-8"><div><span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#F7B500] text-[#061D45]"><Heart className="h-5 w-5 fill-current" /></span><p className="mt-8 font-montserrat text-3xl font-black text-white">Potential comes alive when it is met with care.</p></div></div>}><p className="font-montserrat text-xl font-medium leading-9 text-[#0B2E6B]">“{missionVision.foundersNote}”</p><p className="mt-6 text-xs font-bold uppercase tracking-[0.13em] text-[#079432]">Foundation message</p></EditorialSplit></div></section>

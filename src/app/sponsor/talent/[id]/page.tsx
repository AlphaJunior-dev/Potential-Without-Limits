...
import { ArrowLeft, ArrowRight, FileText, MapPin, MessageCircle, ShieldCheck, Sparkles, Target } from "lucide-react";
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {!loading && record && <>
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <section className="rounded-3xl border border-[#0B2E6B]/10 bg-white p-7 shadow-[0_12px_32px_rgba(5,24,54,0.08)]"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#079432]">Shared media</p>{record.mediaUrls?.length ? <div className="mt-5 space-y-2">{record.mediaUrls.map((mediaUrl, index) => <a key={mediaUrl} href={mediaUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-xl bg-[#F5F6F0] px-4 py-3 text-sm font-bold text-[#079432] transition hover:bg-[#079432] hover:text-white"><span className="inline-flex items-center gap-2"><FileText className="h-4 w-4" /> Media {index + 1}</span><ArrowRight className="h-4 w-4" /></a>)}</div> : <p className="mt-5 rounded-2xl bg-[#F5F6F0] p-4 text-xs leading-5 text-[#0B2E6B]/62">No media has been shared for this private record yet.</p>}</section></aside>
          </div>

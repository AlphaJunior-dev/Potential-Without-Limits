...
export default function TalentsPage() {
  ...
  const filteredTalent = useMemo(() => visibleProfiles.filter((profile) => {
    ...
    const canSee = (field: "summaryVisible" | "ageBandVisible" | "educationLevelVisible" | "regionVisible" | "skillsVisible" | "storyVisible" | "aspirationVisible" | "supportPathwayVisible") => hasApprovedSponsorAccess || visibility?.[field] === true;
    ...
    const matchesSearch = !query || [profile.name, profile.category, canSee("summaryVisible") ? profile.bio : "", canSee("ageBandVisible") ? profile.ageBand : "", canSee("educationLevelVisible") ? profile.educationLevel : "", canSee("regionVisible") ? profile.region : "", canSee("storyVisible") ? profile.story : "", canSee("aspirationVisible") ? profile.aspiration : "", canSee("supportPathwayVisible") ? profile.supportPathway : "", ...(canSee("skillsVisible") ? profile.skills || [] : [])].filter(Boolean).some((value) => String(value).toLowerCase().includes(query));
      ...
      <section className="px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{filteredTalent.map((profile, index) => {
            const visibility = profile.publicVisibility;
            const canSee = (field: "photoVisible" | "summaryVisible" | "ageBandVisible" | "educationLevelVisible" | "regionVisible" | "skillsVisible" | "storyVisible" | "aspirationVisible") => hasApprovedSponsorAccess || visibility?.[field] === true;

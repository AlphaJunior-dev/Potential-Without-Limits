...
const categories = ["All", "Technology", "Music", "Digital Art", "Sports (Football)", "Academics", "Robotics", "Leadership", "Entrepreneurship"];
...
export default function TalentsPage() {
  ...
  const filteredTalent = useMemo(() => profiles.filter((profile) => {
    ...
    const matchesSearch = !query || [profile.name, profile.bio, profile.category, profile.dream, ...(profile.skills || [])].filter(Boolean).some((value) => String(value).toLowerCase().includes(query));
      ...
      <section className="px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-8 border-b border-[#0B2E6B]/10 pb-10 lg:flex-row lg:items-end lg:justify-between">
            <SectionHeading eyebrow="Find an overview" title="Potential, organised with care." intro="Use the filters to explore the public overview categories currently available." />
          ...
          <div className="mt-7 flex gap-2 overflow-x-auto pb-2">{categories.map((category) => <button key={category} onClick={() => setSelectedCategory(category)} className={`shrink-0 rounded-full px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] transition ${selectedCategory === category ? "bg-[#0B2E6B] text-white" : "border border-[#0B2E6B]/12 bg-white text-[#0B2E6B]/68 hover:border-[#079432] hover:text-[#079432]"}`}>{category}</button>)}</div>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{filteredTalent.map((profile, index) => <article key={profile.id} className="group overflow-hidden rounded-[1.75rem] border border-[#0B2E6B]/8 bg-white shadow-[0_14px_40px_rgba(11,46,107,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_56px_rgba(11,46,107,0.14)]"><div className="relative aspect-[4/3] overflow-hidden bg-[#061D45]"><TalentPhoto src={profile.coverPhoto} alt="Sponsor Talent overview" fill className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#061D45]/80 to-transparent p-5"><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#A9F1C3]">{String(index + 1).padStart(2, "0")} / {profile.category || "Sponsor Talent"}</span></div></div><div className="p-6"><div className="flex gap-4"><h2 className="font-montserrat text-2xl font-black tracking-[-0.035em] text-[#0B2E6B]">{profile.name || "Sponsor Talent"}</h2><span className="ml-auto shrink-0 text-[10px] font-bold uppercase tracking-[0.1em] text-[#079432]">{hasApprovedSponsorAccess ? "Private" : "Public"}</span></div><p className="mt-4 line-clamp-3 text-sm leading-6 text-[#0B2E6B]/66">{profile.bio || profile.dream || "An approved non-identifying overview will appear here."}</p><Link href={`/portfolio/${profile.id}`} className="mt-7 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.12em] text-[#079432] hover:text-[#14B84A]">Open overview <ArrowUpRight className="h-4 w-4" /></Link></div></article>)}</div>
          {filteredTalent.length === 0 && <div className="mt-12 rounded-[1.75rem] border border-dashed border-[#0B2E6B]/18 p-12 text-center"><p className="font-montserrat text-2xl font-bold text-[#0B2E6B]">No matching public overview yet.</p><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[#0B2E6B]/65">Try a different search or category. PWLIF administrators control which non-identifying profiles appear here.</p></div>}
        </div>
      </section>
    </div>
  );
